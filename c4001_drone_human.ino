...
void setup()
{
  ...
  radar.setDetectThres(60, 1200, 10);       //range an object is considered detected in (third param is sensitivity, less is more sensitive)
  radar.setDetectionRange(60, 1200, 1200);  //range the c4001 scans in (detects all motion from .6m to 12m)
  radar.setTrigSensitivity(3);              //trigger sensitivity 
  radar.setKeepSensitivity(1);              //keep-locked-on sensitivity (this determines when a moving object is no longer detected)
